...
const SchoolsAnalytics = () => {
  ...
  const schoolPerformance = useMemo(() => {
    ...
    return schools.map((schoolItem) => {
      ...
      const usageByDate = schoolUsageEntries.reduce<Record<string, number>>((accumulator, log) => {
        accumulator[log.date] = (accumulator[log.date] || 0) + log.minutes;
        return accumulator;
      }, {});
      ...
      const avgUsageMinutes = schoolUsageEntries.length
        ? Math.round(schoolUsageEntries.reduce((sum, entry) => sum + entry.minutes, 0) / schoolUsageEntries.length)
        : 0;
  ...
  return (
    <DashboardLayout title="Schools Performance Analytics">
