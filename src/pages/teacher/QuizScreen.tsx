...
const QuizScreen = () => {
  ...
  const [scanning, setScanning] = useState(false);
    ...
    // track each student's selected answer for the current question
    const [answers, setAnswers] = useState<Record<string, string | null>>(Object.fromEntries(classStudents.map(s => [s.id, null])));
    // teacher-selected option for the current question (clickable)
    const [selectedOption, setSelectedOption] = useState<string | null>(null);
    // which student is currently selected in the right panel
  ...
  const simulateScan = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      const opts = ["A", "B", "C", "D"];
      // simulate student answer: 70% chance of correct
      const studentAnswer = Math.random() > 0.3 ? question.correct : opts[Math.floor(Math.random() * 4)];
      const isCorrect = studentAnswer === question.correct;

      setScannedStudents(prev => [...prev, student.id]);
      setAnswers(prev => ({ ...prev, [student.id]: studentAnswer }));
      if (isCorrect) {
  ...
  const nextQuestion = () => {
    if (currentQ < quizQuestions.length - 1) {
      ...
      // reset scanned students and answers for next question
      setScannedStudents([]);
      setLastScan(null);
      setAnswers(Object.fromEntries(classStudents.map(s => [s.id, null])));
    ...
    <DashboardLayout title="Quiz Mode">
        ...
        {!quizStarted ? (
          ...
          <div className="grid lg:grid-cols-3 gap-6">
                      ...
                      <div
                        ...
                        onClick={() => {
                          if (selectedStudentId) {
                            // assign this option to the selected student
                            setAnswers(prev => ({ ...prev, [selectedStudentId]: opt }));
                          } else {
                            // teacher-level selection (no student selected)
                            setSelectedOption(opt);
                          }
                        }}
                        onKeyDown={(e) => {
                          if (e.key === 'Enter' || e.key === ' ') {
                            if (selectedStudentId) setAnswers(prev => ({ ...prev, [selectedStudentId]: opt }));
                            else setSelectedOption(opt);
                          }
                        }}
                        className={`rounded-xl p-3 text-sm font-medium text-center cursor-pointer select-none border ${(
                          selectedStudentId ? answers[selectedStudentId] === opt : selectedOption === opt
                        ) ? 'bg-primary text-primary-foreground border-primary' : 'bg-secondary text-foreground border-border'}`}
                      >
                        {opt}
                      </div>
                    ))}
            ...
            {/* Right sidebar: student responses */}
            <div className="space-y-4">
              <Card className="shadow-card border-border">
                <CardHeader className="pb-2">
                  <CardTitle className="font-display text-sm flex items-center gap-2">
                    <Trophy className="w-4 h-4 text-amber" /> Student Responses
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-2 max-h-[420px] overflow-y-auto">
                    {classStudents.map(s => (
                      <div
                        ...
                        className={`flex items-center justify-between gap-2 p-2 rounded-lg transition-colors ${selectedStudentId === s.id ? 'ring-2 ring-primary/30 bg-primary/5' : 'hover:bg-secondary'}`}
                      >
                        <div className="text-sm text-foreground">{s.rollNo}. {s.name}</div>
                        <div className="flex items-center gap-1">
                          {['A','B','C','D'].map(opt => (
                            <div key={opt} className={`text-xs px-2 py-1 rounded-md border ${answers[s.id] === opt ? 'bg-primary text-primary-foreground border-primary' : 'bg-secondary text-muted-foreground border-border'}`}>
                              {opt}
                            </div>
                          ))}
                        </div>
