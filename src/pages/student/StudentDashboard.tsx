...
import {
  ...
  FileText, CheckCircle2, Award, Medal
} from "lucide-react";
import {
  students, subjects, chapters, studentQuizResults, classStatus,
  dailyUpdates, classes, schools, coCurricularActivities,
  studentAttendance, chapterStatuses, topics, classRecordings,
  studentBadges, studentCertificates
} from "@/data/demo-data";
import {
...
const StudentDashboard = () => {
  ...
  const myResults = studentQuizResults.filter(r => r.studentId === student.id);
  ...
  // compute remaining quizzes based on grade chapters
  const gradeChapters = chapters.filter(ch => ch.grade === grade);
  // class students and ranking
  const classStudents = students.filter(s => s.classId === student.classId);
  const classPerformance = classStudents.map(cs => {
  ...
  const classRank = classPerformance.findIndex(p => p.id === student.id) + 1 || 0;

  // recent quiz list (most recent at top)
  const recentQuizzes = myResults
    .slice()
    .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
    .slice(0, 3);

  const myUpdates = dailyUpdates.filter(u => u.classId === student.classId);
  const myAttendance = studentAttendance.find(a => a.studentId === student.id);

  const gradeSubjects = subjects.filter(s => s.grades.includes(grade));

  // Badges & Certificates
  const myBadges = studentBadges.filter(b => b.studentId === student.id);
  const myCertificates = studentCertificates.filter(c => c.studentId === student.id);


  // Performance chart data
  const subjectPerformance = gradeSubjects.map(sub => {
  ...
  const myChapters = gradeSubjects.flatMap(sub =>
    chapters.filter(ch => ch.subjectId === sub.id && ch.grade === grade)
  );
  const completedChapters = myChapters.filter(ch => chapterStatuses[ch.id] === "completed").length;
  ...
  const myTopics = myChapters.flatMap(ch => topics.filter(t => t.chapterId === ch.id));
