...
import { subjects, chapters, chapterQuizzes, students, classes, studentQuizResults } from "@/data/demo-data";
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 -mx-2 px-2">
            {gradeSubjects.map(sub => {
              ...
              const quizCount = subChapters.reduce(
                (a, ch) => a + chapterQuizzes.filter(q => q.chapterId === ch.id).length, 0
              );
              return (
                <Card
                  key={sub.id}
        ...
        <>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {subjectChapters.map(ch => {
              ...
              const prevResult = studentQuizResults.find(r => r.studentId === student.id && r.chapterId === ch.id);
              return (
